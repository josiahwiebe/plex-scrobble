...
import { getUserByPlexId, updateUser } from '../lib/database.js'
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (event === 'media.scrobble' && settings.events.scrobble) {
      return await handleMarkAsWatched(eventData, user, letterboxdPassword, settings)
    } else if (event === 'media.rate' && settings.events.rate) {
      return await handleRate(eventData, user, letterboxdPassword, settings)
    } else {
...
async function handleRate(eventData: PlexWebhookEvent, user: User, password: string, settings: WebhookSettings) {
  try {
